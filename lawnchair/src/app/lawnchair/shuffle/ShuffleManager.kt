...
class ShuffleManager(
    ...
    private fun performShuffle(dataModel: BgDataModel): Int {
        ...
        // Collect non-widget workspace items and widgets from the in-memory model.
        val allWorkspaceItems = synchronized(dataModel) {
            dataModel.workspaceItemInfos()
        }
        val allWidgets = synchronized(dataModel) {
            dataModel.widgetInfos()
        }
...
/**
 * Non-widget items in the in-memory model (icons, folders, app pairs across all
 * containers). Callers filter by container. Caller must hold the model lock.
 *
 * On 16-dev [BgDataModel] no longer exposes the `workspaceItems`/`appWidgets`
 * lists, so we read [BgDataModel.itemsIdMap] (an `Iterable<ItemInfo>`) directly.
 */
internal fun BgDataModel.workspaceItemInfos(): List<ItemInfo> = itemsIdMap.filter { it !is LauncherAppWidgetInfo }

/** Widget items in the in-memory model. Caller must hold the model lock. */
internal fun BgDataModel.widgetInfos(): List<LauncherAppWidgetInfo> = itemsIdMap.filterIsInstance<LauncherAppWidgetInfo>()
